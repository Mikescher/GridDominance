...
public void CreateNext(TetroField f, int iter)
{
	if (iter == 6) { RegisterSolution(f); return; }

	for (int s = 0; s < TetroPieces.ALPHABET.Length; s++)
	{
		...
		{
			{
				var sf = f.Clone();  // from top
				if (TryInsert(sf, piece, new DPoint(i, 0), new DPoint(0, +1))) CreateNext(sf, iter+1);
			}
			{
				var sf = f.Clone();  // from right
				if (TryInsert(sf, piece, new DPoint(3, i), new DPoint(-1, 0))) CreateNext(sf, iter + 1);
			}
		}
	}
}
