...
namespace GridDominance.Shared.Screens.OverworldScreen.Entities
{
	...
	{
		private const string IAB_CODE = GDConstants.IAB_WORLD4;

		public OverworldNode_W4(GDOverworldScreen scrn, FPoint pos) : base(scrn, pos, Levels.WORLD_004)
		{
			//
		}
		...
		protected override UnlockState IsUnlocked()
		{
			...
			{
				var supplyNodes = Levels.WORLD_003.Nodes.Where(n => n.OutgoingPipes.Any(p => p.Target == Blueprint.ID));
			...
			if (GDConstants.USE_IAB)
			{
				...
				switch (ip)
				{
					...
					default:
						SAMLog.Error("EnumSwitch", "IsUnlocked()", "MainGame.Inst.Bridge.IAB.IsPurchased(MainGame.IAB_WORLD4)) -> " + ip);
						return UnlockState.NeedsPurchase;
				}
			}
			else
		...
		protected override void OnClickDeny()
		{
			...
			else
			{
				...
				Owner.HUD.AddModal(new WorldPreviewPanel(previews, Blueprint.ID, IAB_CODE, 4), true, 0.8f, 1f);
			}
		}
	}
}
