...
{
	public sealed class TrishotCannonBlueprint : ICannonBlueprint
	{
		...
		public TrishotCannonBlueprint(float x, float y, float d, int p, float rot, byte cid, BulletPathBlueprint[] bp)
		{
			X = x;
			Y = y;
			Diameter = d;
		...
		public TrishotCannonBlueprint(float x, float y, float d, int p, float rot, byte cid)
		{
			X = x;
			Y = y;
			Diameter = d;
		...
		{
			bw.Write(LevelBlueprint.SERIALIZE_ID_TRISHOT);
		...
		public static TrishotCannonBlueprint Deserialize(BinaryReader br)
		{
			var i = br.ReadInt32();
			var p = br.ReadInt32();
			var x = br.ReadSingle();
			...
			return new TrishotCannonBlueprint(x, y, d, p, a, (byte)i, b);
		}
	}
}
