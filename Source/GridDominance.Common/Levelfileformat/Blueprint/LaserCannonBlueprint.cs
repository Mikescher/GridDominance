...
namespace GridDominance.Levelfileformat.Blueprint
{
	public sealed class LaserCannonBlueprint : ICannonBlueprint
	{
		...
		public LaserCannonBlueprint(float x, float y, float d, int p, float rot, byte cid, BulletPathBlueprint[] bp)
		{
			X = x;
			Y = y;
			Diameter = d;
		...
		public LaserCannonBlueprint(float x, float y, float d, int p, float rot, byte cid)
		{
			X = x;
			Y = y;
			Diameter = d;
		...
		public static LaserCannonBlueprint Deserialize(BinaryReader br)
		{
			...
			return new LaserCannonBlueprint(x, y, d, p, a, (byte)i, b);
		}
	}
}
