...
{
	public sealed class RelayCannonBlueprint : ICannonBlueprint
	{
		...
		public RelayCannonBlueprint(float x, float y, float d, int p, float rot, byte cid, BulletPathBlueprint[] bp)
		{
			X = x;
			Y = y;
			Diameter = d;
		...
		public RelayCannonBlueprint(float x, float y, float d, int p, float rot, byte cid)
		{
			X = x;
			Y = y;
			Diameter = d;
		...
		{
			bw.Write(LevelBlueprint.SERIALIZE_ID_RELAY);
		...
		public static RelayCannonBlueprint Deserialize(BinaryReader br)
		{
			var i = br.ReadInt32();
			var p = br.ReadInt32();
			var x = br.ReadSingle();
			...
			return new RelayCannonBlueprint(x, y, d, p, a, (byte)i, b);
		}
	}
}
