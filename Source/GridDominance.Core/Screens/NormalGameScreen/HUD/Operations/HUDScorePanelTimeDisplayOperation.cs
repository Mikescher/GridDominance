...
{
	class HUDScorePanelTimeDisplayOperation : CyclicOperation<HUDScorePanel>
	{
		...
		protected override void OnCycle(HUDScorePanel element, int counter)
		{
			...
			{
				_mode = !_mode;
				element.AddOperation(new HUDScorePanelTimeChangeOperation(_text2_1, _text2_2));
			}
			else
			{
				_mode = !_mode;
				element.AddOperation(new HUDScorePanelTimeChangeOperation(_text1_1, _text1_2));
			}
		}
	}
}
